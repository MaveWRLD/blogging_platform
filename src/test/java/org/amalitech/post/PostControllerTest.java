...
class PostControllerTest {
    ...
    void setUp() {
        ...
        testPost = new Post("Test Title", "Test Body", org.amalitech.post.PostStatus.draft, Instant.now(), null);
        testPost.setId(1);
        ...
        testPostDto = new PostDto(
                ...
                10,
                3,
                org.amalitech.post.PostStatus.draft,
                1L,
                "testuser"
        );
    ...
    void updatePost_WithValidRequest_ShouldReturnUpdatedPost() throws Exception {
        Post updatedPost = new Post("Updated Title", "Updated Body", org.amalitech.post.PostStatus.published, Instant.now(), Instant.now());
        updatedPost.setId(1);
        ...
        PostDto updatedPostDto = new PostDto(
                ...
                15,
                5,
                org.amalitech.post.PostStatus.published,
                1L,
                "testuser"
        );
    ...
    void getPost_WithValidId_ShouldReturnPostWithComments() throws Exception {
        when(postService.findPostById(anyInt())).thenReturn(testPost);
        when(commentService.getCommentsByPostId(anyInt())).thenReturn(new ArrayList<>());
        when(postMapper.toDto(any(Post.class))).thenReturn(testPostDto);
