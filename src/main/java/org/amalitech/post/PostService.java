...
@Service
@AllArgsConstructor
public class PostService implements PostExistenceChecker {
    ...
    /**
     * Get a post by ID. Callers that need comments too (e.g. the "post with
     * comments" endpoint) compose that separately via the Comment feature's
     * own public service - Post never depends on Comment.
     */
    ...
    /**
     * Public port for other features (e.g. Comment) to check a post exists
     * without reaching into PostRepository directly.
     */
    @Override
    @Transactional(readOnly = true)
    ...
    @Transactional
    @PreAuthorize("@postAuthorizationService.canUpdatePost(#postId)")
    @Caching(evict = {
     ...
     */
    @Transactional(isolation = Isolation.READ_COMMITTED)
    @PreAuthorize("@postAuthorizationService.canDeletePost(#id)")
    @Caching(evict = {
